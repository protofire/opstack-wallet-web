...
> = {
  SIGNATURE_REQUEST: (appInfo: AppInfo) => ({
    ...
    options: {
      body: `${appInfo.name} wants you to sign a message. Open the Superchain Safe to continue.`,
    },
  }),
  TRANSACTION_REQUEST: (appInfo: AppInfo) => ({
    title: 'Transaction request',
    options: {
      body: `${appInfo.name} wants to submit a transaction. Open the Superchain Safe to continue.`,
    },
  }),
}
